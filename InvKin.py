...
def invKin(lengths, destination):    
    ...
    rall = np.matmul(np.matmul(Rz(destrz), Ry(destry)), Rx(destrx))

    #define points to be transformed
    act5pos = [[0.0, float(-length56)],
               [0.0, 0.0],
               [0.0, 0.0]]
    ...
    joint5toorigin = ((0.0, act5newpos[0][1]),
                      (0.0, act5newpos[1][1]),
                      (act5newpos[2][1], act5newpos[2][1]))

    #cannot divide by zero so catch and replace with real value
    try:
        angle = np.arctan((joint5toorigin[1][1] - joint5toorigin[1][0]) / (joint5toorigin[0][1] - joint5toorigin[0][0]))
    ...
    try:
        theta1 = np.arccos((length2**2 + targetpos[0]**2 + (targetpos[1]-length1)**2 - length34**2)/
                           (2 * length2 * np.sqrt(targetpos[0]**2 + (targetpos[1]-length1)**2)))
        theta2 = np.pi - np.arccos((length2**2 + length34**2 - (targetpos[0]**2 + (targetpos[1]-length1)**2))/
                                   (2 * length2 * length34))
    ...
    point41 = pointToPlane(np.array([xplots[1], yplots[1], zplots[1]]), origin4, normal4)
    point42 = pointToPlane(np.array([xplots[6], yplots[6], zplots[6]]), origin4, normal4)
    ...
    point61 = pointToPlane(np.array([xplots[2], yplots[2], zplots[2]]), origin6, normal6)
    point62 = pointToPlane(findEndEffector(desttx, destty, desttz, destrx, destry, destrz, 50), origin6, normal6)
    ...
    return ((-angle1,-angle2,angle3,angle4,angle5,angle6),tuple(xplots),tuple(yplots),tuple(zplots))
...
def main():
    ...
    destination = [randint(-200,200),randint(-200,200),randint(0,200),randint(-6,6),randint(-6,6),randint(-6,6)]
    ...
    plotOrigin(xplots[0], yplots[0], zplots[0], 0, 0, 0, 30)
    plotOrigin(destination[0], destination[1], destination[2], destination[3], destination[4], destination[5], 30)
